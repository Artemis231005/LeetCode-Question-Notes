...
This is the same problem statement as LC 3903 (Smallest Stable Index I), but with larger constraints — the `O(n²)` brute force is no longer fast enough, making the two-pass `O(n)` approach mandatory rather than optional.
...
* Given this problem's larger constraints (Medium difficulty vs. LC 3903's Easy), this approach will likely time out — it's included mainly to establish correctness before optimizing.
...
```text
i=0: left = max(-inf, 5) = 5 → score = 5 - right[0] = 5 - 0 = 5 → 5 > 3
...
* Given the larger constraints on this version of the problem, this `O(n)` two-pass approach is required, not just preferred — the identical brute force from LC 3903 would time out here.
